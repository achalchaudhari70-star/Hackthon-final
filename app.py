...
with st.sidebar:
    ...
    choice_type = st.selectbox(
        ...
        ["Heart Disease", "Diabetes", "Kidney Disease", "Common Illness", "Health Chatbot"]
    )
...
if choice_type == "Heart Disease":
    ...
    if st.button('Predict'):
        ...
        if prediction[0] == 1:
            st.error("Heart Disease: High Risk")
        else:
            st.success("Heart Disease: Low Risk")
...
# Diabetes Prediction Section
    ...
    if st.button('Predict'):
            ...
            if predicted[0] == 0:
                st.success("Diabetes : Low Risk ")  
            else:
                st.error("Diabetes : High Risk ")
...
# Kidney Disease Prediction Section
    ...
    if st.button("Predict"):
        ...
        # Display prediction
        if prediction == 1:
            st.error("Chronic Kidney Disease : High Risk")
        else:
            st.success("Chronic Kidney Disease : Low Risk")
...
elif choice_type == "Health Chatbot":
    st.header("AI Chatbot")
    user_query = st.text_input("Ask Health Related Question :")
    if st.button("Get Response"):
        try:
            model_groq = init_chat_model("groq:llama-3.1-8b-instant")
            response = model_groq.invoke(user_query)
